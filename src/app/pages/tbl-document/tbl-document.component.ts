...
export class Tbl_documentComponent implements OnInit {
  ...
  formTitle: string = 'เพิ่มข้อมูล-เอกสารการซ่อม ';
  ...
  ngOnInit() {
	...
	*/
    ...
    this.myForm = this.fb.group({
      ...
      DOC_START_DATE: ['', Validators.required],
      DOC_END_DATE: ['', Validators.required],
      COST_ID: ['', Validators.required],
      DOC_LOCATION_REF: ['', Validators.required],
      DOC_REFERENCE_COM: ['', Validators.required],
      DOC_REFERENCE_DOC: ['', Validators.required],
      DOC_ATTACH: ['', Validators.required],
      DOC_REMARK: ['', Validators.required],
      DOC_STATUS: ['', Validators.required],
      USER_CREATE: ['', Validators.required],
      USER_UPDATE: ['', Validators.required],
      CREATE_DATE: ['', Validators.required],
      UPDATE_DATE: ['', Validators.required],
      HW_WARANTEE: ['', Validators.required],
      HW_WAR_YEAR: ['', Validators.required],
      DOC_GROUP: ['', Validators.required],
      COST_SECTION: ['', Validators.required],
      upsize_ts: ['', Validators.required],
    });
  }
  ...
  getByID(id) {
    ...
    this.apiService.getById(this.ModelName, id).subscribe((response: any) => {
      ...
      this.myForm.get('DOC_ID').setValue(response.DOC_ID);
      this.myForm.get('DOC_NO').setValue(response.DOC_NO);
      this.myForm.get('DOC_DATE').setValue(response.DOC_DATE);
      this.myForm.get('DOC_YEAR').setValue(response.DOC_YEAR);
      this.myForm.get('DOC_DESCRIPTION').setValue(response.DOC_DESCRIPTION);
      this.myForm.get('DOC_TYPE').setValue(response.DOC_TYPE);
      this.myForm.get('SUPPLIER').setValue(response.SUPPLIER);
      this.myForm.get('DOC_START_DATE').setValue(response.DOC_START_DATE);
      this.myForm.get('DOC_END_DATE').setValue(response.DOC_END_DATE);
      this.myForm.get('COST_ID').setValue(response.COST_ID);
      this.myForm.get('DOC_LOCATION_REF').setValue(response.DOC_LOCATION_REF);
      this.myForm.get('DOC_REFERENCE_COM').setValue(response.DOC_REFERENCE_COM);
      this.myForm.get('DOC_REFERENCE_DOC').setValue(response.DOC_REFERENCE_DOC);
      this.myForm.get('DOC_ATTACH').setValue(response.DOC_ATTACH);
      this.myForm.get('DOC_REMARK').setValue(response.DOC_REMARK);
      this.myForm.get('DOC_STATUS').setValue(response.DOC_STATUS);
      this.myForm.get('USER_CREATE').setValue(response.USER_CREATE);
      this.myForm.get('USER_UPDATE').setValue(response.USER_UPDATE);
      this.myForm.get('CREATE_DATE').setValue(response.CREATE_DATE);
      this.myForm.get('UPDATE_DATE').setValue(response.UPDATE_DATE);
      this.myForm.get('HW_WARANTEE').setValue(response.HW_WARANTEE);
      this.myForm.get('HW_WAR_YEAR').setValue(response.HW_WAR_YEAR);
      this.myForm.get('DOC_GROUP').setValue(response.DOC_GROUP);
      this.myForm.get('COST_SECTION').setValue(response.COST_SECTION);
      this.myForm.get('upsize_ts').setValue(response.upsize_ts);
